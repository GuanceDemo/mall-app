<template>
	<view class="content b-t">
		<view class="list b-b" v-for="(item, index) in addressList" :key="index" @click="checkAddress(item)">
			<view class="wrapper">
				<view class="address-box">
					<text v-if="item.defaultStatus==1" class="tag">{{ global.language.default }}</text>
		...
		<button class="add-btn" @click="addAddress('add')">{{ global.language.addAddress }}</button>
	</view>
</template>
		...
		methods: {
			...
			handleDeleteAddress(id){
				...
				uni.showModal({
				    ...
				    content: global.language.delAddress,
