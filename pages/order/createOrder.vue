	...
	<view>
		...
		<view class="yt-list">
			<view class="yt-list-cell b-b" @click="toggleMask('show')">
				<view class="cell-icon">
					{{global.language.shortCoupons}}
				</view>
				<text class="cell-tit clamp">{{global.language.coupons}}</text>
				<text class="cell-tip active">
					{{global.language.selectCoupon}}
				</text>
				<text class="cell-more wanjia wanjia-gengduo-d"></text>
			</view>
			<view class="yt-list-cell b-b">
				<view class="cell-icon hb">
					{{global.language.shortBonusPoints}}
				</view>
				<text class="cell-tit clamp">{{global.language.bonusPointDeduction}}</text>
				<input class="integration" type="number" v-model="useIntegration" :placeholder="global.language.usedPoints" placeholder-class="placeholder"
		...
		<view class="yt-list">
			...
			<view class="yt-list-cell desc-cell">
				...
				<input class="desc" type="text" v-model="desc" :placeholder="global.language.writeRemarks" placeholder-class="placeholder" />
			</view>
		</view>
		...
		<view class="footer">
			<view class="price-content">
				<text>{{global.language.netPayment}}</text>
				<text class="price-tip">￥</text>
				<text class="price">{{calcAmount.payAmount}}</text>
			</view>
			<text class="submit" @click="submit">{{global.language.submitOrder}}</text>
		</view>
		...
		methods: {
			...
			submit() {
				...
				generateOrder(orderParam).then(response => {
					...
					uni.showModal({
						...
						content: global.language.payNowText,
						confirmText: global.language.toPay,
			...
			handleIntegrationInput(event) {
				if (event.detail.value > this.memberIntegration) {
					...
					uni.showToast({
						title: `${global.language.pointsText} ${this.memberIntegration}`,
						duration: 1000
					})
				}
			},
