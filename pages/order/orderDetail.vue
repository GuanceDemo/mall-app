	...
	<view>
		...
		<view class="goods-section">
			<view class="g-header b-b">
				<text class="name">{{global.language.productInformation}}</text>
			</view>
			<!-- 商品列表 -->
		...
		<view class="yt-list">
			<view class="yt-list-cell b-b">
				<text class="cell-tit clamp">{{global.language.totalOfCommodities}}</text>
				<text class="cell-tip">￥{{order.totalAmount}}</text>
			</view>
			<view class="yt-list-cell b-b">
				<text class="cell-tit clamp">{{global.language.freightCharges}}</text>
				<text class="cell-tip">￥{{order.freightAmount}}</text>
			</view>
			<view class="yt-list-cell b-b">
				<text class="cell-tit clamp">{{global.language.eventOffers}}</text>
				<text class="cell-tip red">-￥{{order.promotionAmount}}</text>
			</view>
			<view class="yt-list-cell b-b">
				<text class="cell-tit clamp">{{global.language.coupons}}</text>
				<text class="cell-tip red">-￥{{order.couponAmount}}</text>
			</view>
			<view class="yt-list-cell b-b">
				<text class="cell-tit clamp">{{global.language.bonusPointDeduction}}</text>
				<text class="cell-tip red">-￥{{order.integrationAmount}}</text>
			</view>
			<view class="yt-list-cell desc-cell">
				<text class="cell-tit clamp">{{global.language.remarks}}</text>
		...
		<view class="yt-list">
			<view class="yt-list-cell b-b">
				<text class="cell-tit clamp">{{global.language.orderNumber}}</text>
				<text class="cell-tip">{{order.orderSn}}</text>
			</view>
			<view class="yt-list-cell b-b">
				<text class="cell-tit clamp">{{global.language.submittedTime}}</text>
				<text class="cell-tip">{{order.createTime | formatDateTime}}</text>
			</view>
			<view class="yt-list-cell b-b">
				<text class="cell-tit clamp">{{global.language.modeOfPayment}}</text>
			...
			<view class="yt-list-cell b-b" v-if="order.status==1||order.status==2||order.status==3">
				<text class="cell-tit clamp">{{global.language.paidAmount}}</text>
				<text class="cell-tip">￥{{order.payAmount}}</text>
			</view>
			<view class="yt-list-cell b-b" v-if="order.status==1||order.status==2||order.status==3">
				<text class="cell-tit clamp">{{global.language.timeOfPayment}}</text>
		...
		<view class="footer" v-if="order.status==0||order.status==2||order.status==3">
			<view class="action-box b-t" v-if="order.status==0">
				<button class="action-btn" @click="cancelOrder(order.id)">{{global.language.cancelOrder}}</button>
				<button class="action-btn recom" @click="payOrder(order.id)">{{global.language.payNow}}</button>
			</view>
			<view class="action-box b-t" v-if="order.status == 2">
				<button class="action-btn">{{global.language.viewLogistics}}</button>
				<button class="action-btn recom" @click="receiveOrder(order.id)">{{global.language.confirmReceipt}}</button>
			</view>
			<view class="action-box b-t" v-if="order.status == 3">
				<button class="action-btn">{{global.language.afterSalesService}}</button>
				<button class="action-btn recom">{{global.language.reviewingProduct}}</button>
			</view>
			<view class="price-content" v-if="order.status==0">
				<text>{{global.language.amountPayable}}</text>
	...
	export default {
		...
		filters: {
			...
			formatPayType(payType) {
				if (payType == 0) {
					return global.language.unpaid;
				} else if (payType == 1) {
					return global.language.AlipayPayment;
				} else if (payType == 2) {
					return global.language.WeChatPay;
				}
				return null;
			},
		},
		methods: {
			...
			cancelOrder(orderId) {
				...
				uni.showModal({
					...
					content: global.language.cancelOrderText,
					success: function(res) {
						if (res.confirm) {
							uni.showLoading({
								title: global.language.pleaseWait
							})
							cancelUserOrder({
								orderId: orderId
							}).then(response => {
							...
							console.log(global.language.userClickText);
						}
					}
				});
			},
			...
			receiveOrder(orderId) {
				...
				uni.showModal({
					title: global.language.tips,
					content:global.language.confirmReceiptText,
					success: function(res) {
						if (res.confirm) {
							uni.showLoading({
								title: global.language.pleaseWait
			...
			setOrderStatus(status) {
				switch (status) {
					case 0:
						this.orderStatus = {
							text: global.language.waitingPayment,
							image: '/static/icon_wait.png'
						}
						break;
					case 1:
						this.orderStatus = {
							text: global.language.pendingShipment,
							image: '/static/icon_deliver.png'
						}
						break;
					case 2:
						this.orderStatus = {
							text: global.language.pendingReceipt,
							image: '/static/icon_receive.png'
						}
						break;
					case 3:
						this.orderStatus = {
							text: global.language.transactionCompleted,
							image: '/static/icon_finish.png'
						}
						break;
					case 4:
						this.orderStatus = {
							text: global.language.closeDeal,
