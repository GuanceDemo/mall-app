				...
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData('add')">
					...
					<view v-for="(item,index) in orderList" :key="index" class="order-item">
						...
						<view class="price-box">
							...
							{{global.language.piece}}{{global.language.goods}} {{global.language.netPayment}}
							<text class="price">{{item.payAmount}}</text>
						</view>
						<view class="action-box b-t" v-if="item.status == 0">
							<button class="action-btn" @click="cancelOrder(item.id)">{{global.language.cancelOrder}}</button>
							<button class="action-btn recom" @click="payOrder(item.id)">{{global.language.payNow}}</button>
						</view>
						<view class="action-box b-t" v-if="item.status == 2">
							<button class="action-btn" >{{global.language.viewLogistics}}</button>
							<button class="action-btn recom" @click="receiveOrder(item.id)">{{global.language.confirmReceipt}}</button>
						</view>
	...
	export default {
		...
		data() {
			return {
				...
				navList: [{
						state: -1,
						text: global.language.all
					},
					{
						state: 0,
						text: global.language.pendingPayment
					},
					{
						state: 2,
						text: global.language.waitReceived
					},
					{
						state: 3,
						text: global.language.completed
					},
					{
						state: 4,
		...
		filters: {
			formatStatus(status) {
				...
				switch (+status) {
					case 0:
						statusTip = global.language.waitingPayment;
						break;
					case 1:
						statusTip = global.language.pendingShipment;
						break;
					case 2:
						statusTip = global.language.pendingReceipt;
						break;
					case 3:
						statusTip = global.language.transactionCompleted;
						break;
					case 4:
						statusTip = global.language.closeDeal;
						break;
				}
				return statusTip;
			},
		...
		methods: {
			...
			deleteOrder(orderId) {
				...
				uni.showModal({
				    ...
				    content: global.language.deleteOrderText,
