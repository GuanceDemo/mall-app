<template>
	<view class="container">
        ...
        {{ global.language.empty }}
				<navigator class="navigator" v-if="hasLogin" url="../index/index" open-type="switchTab">{{ global.language.strollingAround }}></navigator>
			</view>
			<view v-else class="empty-tips">
        {{ global.language.empty }}
				<view class="navigator" @click="navToLogin">{{ global.language.toLog }}></view>
			</view>
		</view>
		<view v-else>
			<!-- 列表 -->
            ...
            {{ global.language.clear }}
				...
				<button type="primary" class="no-border confirm-btn" @click="createOrder">{{ global.language.checkout }}</button>
			</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			createOrder() {
				...
				if(cartIds.length==0){
					uni.showToast({
						title: global.language.notSelectedProduct,
						duration:1000
					})
					return;
				}
