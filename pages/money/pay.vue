<template>
  <view class="app">
    <view class="price-box">
      <text>{{ global.language.paymentAmount }}</text>
    ...
    <view class="pay-type-list">
      <view class="type-item b-b" @click="changePayType(1)">
        ...
        <view class="con">
          <text class="tit">{{ global.language.alipayPayment }}</text>
          <text>{{ global.language.recommendedAlipay }}</text>
        </view>
        <label class="radio">
      ...
      <view class="type-item b-b" @click="changePayType(2)">
        ...
        <view class="con">
          <text class="tit">{{ global.language.weChatPay }}</text>
        </view>
        <label class="radio">
    ...
    <text class="mix-btn" @click="confirm">{{ global.language.confirmPayment }}</text>
  </view>
</template>
  ...
  methods: {
    ...
    confirm: async function () {
      if (USE_ALIPAY) {
        if (this.payType != 1) {
          uni.showToast({
            title: global.language.wechatNotSupported,
            icon: "none"
          })
          return;
        }
