<template>
	<view class="container">
		...
		<view class="introduce-section">
			...
			<view class="bot-row">
				<text>{{global.language.salesVolume}}: {{product.sale}}</text>
				<text>{{global.language.stocks}}: {{product.stock}}</text>
				<text>{{global.language.pageviews}}: 768</text>
			</view>
		</view>
		...
		<view class="share-section" @click="share">
			<view class="share-icon">
				...
				{{global.language.rebate}}
			</view>
			<text class="tit">{{global.language.shareRebate}}</text>
			<text class="yticon icon-bangzhu1"></text>
			<view class="share-btn">
				{{global.language.shareNow}}
		...
		<view class="c-list">
			<view class="c-row b-b" @click="toggleSpec">
				<text class="tit">{{global.language.purchaseType}}</text>
				<view class="con">
			...
			<view class="c-row b-b" @click="toggleAttr">
				<text class="tit">{{global.language.offeringParameters}}</text>
				<view class="con">
					<text class="con t-r">{{global.language.check}}</text>
				</view>
				<text class="yticon icon-you"></text>
			</view>
			<view class="c-row b-b" @click="toggleCoupon('show')">
				<text class="tit">{{global.language.coupons}}</text>
				<text class="con t-r red">{{global.language.getCoupon}}</text>
				<text class="yticon icon-you"></text>
			</view>
			<view class="c-row b-b">
				<text class="tit">{{global.language.promotionalActivity}}</text>
				<view class="con-list">
					<text v-for="item in promotionTipList" :key="item">{{item}}</text>
				</view>
			</view>
			<view class="c-row b-b">
				<text class="tit">{{global.language.service}}</text>
		...
		<view class="eva-section">
			<view class="e-header">
				<text class="tit">{{global.language.opinion}}</text>
				<text>(86)</text>
				<text class="tip">{{global.language.praiseRate}} 100%</text>
				<text class="yticon icon-you"></text>
			</view>
			<view class="eva-box">
				...
				<view class="right">
					...
					<text class="con">{{global.language.opinionText}}</text>
					<view class="bot">
						<text class="attr">{{global.language.buyTypeText}}</text>
		...
		<view class="brand-info">
			<view class="d-header">
				<text>{{global.language.brandInformation}}</text>
			</view>
			<view class="brand-box" @click="navToBrandDetail()">
				...
				<view class="title">
					...
					<text>{{global.language.brandInitials}}：{{brand.firstLetter}}</text>
				</view>
			</view>
		</view>
		...
		<view class="detail-desc">
			<view class="d-header">
				<text>{{global.language.textDetails}}</text>
			</view>
		...
		<view class="page-bottom">
			<navigator url="/pages/index/index" open-type="switchTab" class="p-b-btn">
				...
				<text>{{global.language.home}}</text>
			</navigator>
			<navigator url="/pages/cart/cart" open-type="switchTab" class="p-b-btn">
				<text class="yticon icon-gouwuche"></text>
				<text>{{global.language.cartUpper}}</text>
			</navigator>
			<view class="p-b-btn" :class="{active: favorite}" @click="toFavorite">
				<text class="yticon icon-shoucang"></text>
				<text>{{global.language.collections}}</text>
			</view>

			<view class="action-btn-group">
				<button type="primary" class=" action-btn no-border buy-now-btn" @click="buy">{{global.language.buyNow}}</button>
				<button type="primary" class=" action-btn no-border add-cart-btn" @click="addToCart">{{global.language.addCart}}</button>
			</view>
		</view>
		...
		<view class="popup spec" :class="specClass" @touchmove.stop.prevent="stopPrevent" @click="toggleSpec">
			...
			<view class="layer attr-content" @click.stop="stopPrevent">
				<view class="a-t">
					...
					<view class="right">
						<text class="price">¥{{product.price}}</text>
						<text class="stock">{{global.language.stocks}}：{{product.stock}}{{global.language.piece}}</text>
						<view class="selected">
				...
				<button class="btn" @click="toggleSpec">{{global.language.finished}}</button>
			</view>
		</view>
		<!-- 属性-模态层弹窗 -->
				...
				<view class="coupon-item" v-for="(item,index) in couponList" :key="index" @click="addCoupon(item)">
					<view class="con">
						...
						<view class="right">
							...
							<text>{{global.language.fulling}}{{item.minPoint}}{{global.language.available}}</text>
						</view>
...
<script>
	...
	const defaultServiceList = [{
		id: 1,
		name: global.language.HassleFreeReturns
	}, {
		id: 2,
		name: global.language.quickRefund
	}, {
		id: 3,
		name: global.language.freePostage
	}];
	const defaultShareList = [{
			type: 1,
			icon: '/static/temp/share_wechat.png',
			text: global.language.WeChatFriends
		},
		{
			type: 2,
			icon: '/static/temp/share_moment.png',
			text: global.language.friendCircle
		},
		{
			type: 3,
			icon: '/static/temp/share_qq.png',
			text: global.language.QQFriends
		},
		{
			type: 4,
			icon: '/static/temp/share_qqzone.png',
			text: global.language.QQSpace
		}
	]
	export default {
		components: {
          ...
          return item;
			...
			toggleCoupon(type) {
				fetchProductCouponList(this.product.id).then(response => {
					...
					if(this.couponList==null||this.couponList.length==0){
						uni.showToast({
							title:global.language.noCouponsAvailable,
							icon:"none"
						})
						return;
					}
			...
			addCoupon(coupon) {
				...
				addMemberCoupon(coupon.id).then(response => {
					uni.showToast({
						title: global.language.couponGetSuccess,
						duration: 2000
					});
				});
			},
			...
			toFavorite() {
				...
				if (this.favorite) {
					...
					deleteProductCollection({
						...
						uni.showToast({
							title: global.language.removedFavoritesSuccess,
							icon: 'none'
						});
					...
					createProductCollection(productCollection).then(response => {
						uni.showToast({
							title: global.language.collectionSucceeded,
							icon: 'none'
						});
						this.favorite = !this.favorite;
					});
			...
			buy() {
				uni.showToast({
					title: global.language.orderOnlyFromCart,
					icon: 'none'
				});
			},
			stopPrevent() {},
				...
				for (let i = 0; i < data.productAttributeList.length; i++) {
					...
					if (item.type == 0) {
						...
						if (item.handAddStatus == 1) {
							...
							let inputList = filterValueList[0] && filterValueList[0].value && filterValueList[0].value.split(',');
							let inputListlength = inputList&&inputList.length || 0
							for (let j = 0; j < inputListlength; j++) {
			...
			initPromotionTipList(data) {
				...
				if (promotionType == 0) {
					this.promotionTipList.push(global.language.noOffers);
				} else if (promotionType == 1) {
					this.promotionTipList.push(global.language.singleDiscount);
				} else if (promotionType == 2) {
					this.promotionTipList.push(global.language.memberBenefits);
				} else if (promotionType == 3) {
					this.promotionTipList.push(global.language.multiBuyDiscount);
					for (let item of data.productLadderList) {
						this.promotionTipList.push(global.language.fulling+' ' + item.count + ' ' + global.language.piece+' '+global.language.at+' ' + item.discount * 10 + global.language.discount);
					}
				} else if (promotionType == 4) {
					this.promotionTipList.push(global.language.fullDiscount);
					for (let item of data.productFullReductionList) {
						this.promotionTipList.push(global.language.fulling+' ' + item.fullPrice + global.language.yuan+" " +global.language.minus+ item.reducePrice + global.language.yuan);
					}
				} else if (promotionType == 5) {
					this.promotionTipList.push(global.language.limitedTimeOffer);
				}
			},
			//初始化商品详情信息
			initProductDesc() {
			...
			addToCart() {
				...
				let cartItem = {
					...
					productAttr: productSkuStock&&productSkuStock.spData||"",
					productBrand: this.product.brandName,
					productCategoryId: this.product.productCategoryId,
					productId: this.product.id,
					productName: this.product.name,
					productPic: this.product.pic,
					productSkuCode: productSkuStock && productSkuStock.skuCode||"",
					productSkuId: productSkuStock && productSkuStock.id||"",
				...
				addCartItem(cartItem).then(response => {
					uni.showToast({
						title: this.menusTitle(response.message),
						duration: 1500
					})
				});
			},
			...
			checkForLogin() {
				if (!this.hasLogin) {
					uni.showModal({
						title: global.language.tips,
						content: global.language.loginText,
						confirmText: global.language.goLogin,
						...
						success: function(res) {
							if (res.confirm) {
								...
								console.log(global.language.clickCancel);
							}
						}
					});
					return false;
